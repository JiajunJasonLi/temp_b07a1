...
public class SavingsAccount extends Account {
  ...
  public SavingsAccount(int id, String name, BigDecimal balance) throws ConnectionFailedException {
    ...
    // Find the type ID of an SavingsAccount
    List<Integer> allTypeIds = DatabaseSelectHelper.getAccountTypesIds();
    
    try {
      // Iterate over list and find ID corresponding to SavingsAccount type
      for (Integer currentTypeId: allTypeIds) {
        if (DatabaseSelectHelper.getAccountTypeName(currentTypeId).equalsIgnoreCase("SAVINGS")) {
